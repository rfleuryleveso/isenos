...
void VMTVa2Indexes (uint64_t virtual_address, uint64_t *pml4index, uint64_t *pdptindex, uint64_t *pdindex)
{
 ...
 * @return
 */
uint16_t VMTGetPdptIndex (uint8_t filled_pml4[512], uint16_t pml4_index)
{
  int index = 0;
  for (int i = 0; i < 512; i++)
	{
 ...
 * @return
 */
uint16_t VMTGetPdtIndex (uint8_t filled_pdp[512][512], uint16_t pml4_index, uint16_t pdp_index)
{
  int index = 0;
  for (int i = 0; i < 512; i++)
	{
...
void VMTComputeTablesCounts (uint64_t *pd_table_count, uint64_t *pdp_table_count,
	  ...
	  if ((allocation->flags & PAMA_FLAG_PRESENT) == PAMA_FLAG_PRESENT)
		{
		  ...
		  VMTVa2Indexes (virtual_address, &va_pml4_index, &va_pdpt_index, &va_pd_index);
 ...
 */
void VMTPopulatePageTableForAddress (
...
)
{
  ...
  VMTVa2Indexes (virtual_address, &va_pml4_index, &va_pdp_index, &va_pd_index);
  ...
  uint64_t pdp_table_index = VMTGetPdptIndex (filled_pml4, va_pml4_index);
  uint64_t pd_table_index = VMTGetPdtIndex (filled_pdp, va_pml4_index, va_pdp_index);
...
void VMTPopulatePageTable (void *base, uint64_t pdp_table_count, uint64_t pd_table_count, uint8_t filled_pml4[512],
						   uint8_t filled_pdp[512][512], int64_t offset)
{
  ...
  void *pdp_table_top = pml4_table_base + sizeof (PAGE_MAP_AND_DIRECTORY_POINTER) * 512 * pdp_table_count;

  void *pd_table_base = pdp_table_top;
  void *pd_table_top = pd_table_base + sizeof (PAGE_MAP_AND_DIRECTORY_POINTER) * 512 * pd_table_count;
	  ...
	  if ((allocation->flags & PAMA_FLAG_PRESENT) == PAMA_FLAG_PRESENT)
		{
		  ...
		  VMTPopulatePageTableForAddress (virtual_address, physical_address, pml4_table_base, pml4_table_top, pdp_table_base, pdp_table_top, pd_table_base, pd_table_top, filled_pml4, filled_pdp, offset);
		}
	}
...
uint64_t VMTMUpdate (uint8_t identity_mapped_memory)
{
  ...
  uint8_t filled_pml4[512];
  uint8_t filled_pdp[512][512];
  MemoryZero ((void *)filled_pdp, 512 * 512);
  MemoryZero ((void *)filled_pml4, 512);

  VMTComputeTablesCounts (&pd_table_count, &pdp_table_count, filled_pml4, filled_pdp);
  ...
  uint64_t tables_page_physical = PAMFindFreePages (pages_count);
  uint64_t tables_page_virtual = MemPhysToVirt (tables_page_physical);
  ...
  VMTPopulatePageTable ((void*)tables_page_physical, pdp_table_count, pd_table_count, filled_pml4, filled_pdp, 0);
